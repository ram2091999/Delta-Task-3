    ...
    console.log(err);
      ...
      docs.forEach(function(e){
          ...
          for(let k=0;k<e.formsArray.length;k++){
            ...
            if(isActive(e.formsArray[k]))
            otherUserForms.push(e.formsArray[k]);}
          }

      });
      //console.log(otherUserForms);
      res.render("userHome",{user:currrentUser,docs:[... new Set(otherUserForms)]});
    }
...
app.post("/signup",[check("password").isLength({min:5}).withMessage("Password must have atleast 5 characters"),check('confirmPassword', 'Passwords do not match').custom((value, {req}) => (value === req.body.password))],function(req,res){
